...
char* indexes(const char* path)
{
    ...
    char* temppath = malloc(strlen(path) + 12);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    while (true)
    {
        buffer[i] = (char) fgetc(file); // store the stream
        if (feof(file) != 0) break; // reached end of file so get out of loop
        i++;
        buffer = (char*) realloc(buffer, sizeof(char) * (i + 1)); // increase buffer
    }
    *content = buffer; // store address of first byte, but dereferenced
    *length = i; //  store number of bytes
    return true;
}
...
const char* lookup(const char* path)
{
    // first find the location of the last fullstop in path
    const char ch = '.';
    char *ret;
    char* temp;
    ...
    if (strcasecmp(ret,".css") == 0)
    {
       temp = "text/css";
    }
    else if (strcasecmp(ret,".html") == 0)
    {
        temp = "text/html";
    }
    else if (strcasecmp(ret,".gif") == 0)
    {
        temp = "image/gif";
    }
    else if (strcasecmp(ret,".ico") == 0)
    {
        temp = "image/x-icon";
    }
    else if (strcasecmp(ret,".jpg") == 0)
    {
        temp = "image/jpeg";
    }
    else if (strcasecmp(ret,".js") == 0)
    {
        temp = "text/javascript";
    }
    else if (strcasecmp(ret,".php") == 0)
    {
        temp = "text/x-php";
    }
    else if (strcasecmp(ret,".png") == 0)
    {
        temp = "image/png";
    }
    else
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // first determine the method
    needle = strchr(haystack, 32); // find the first space: ASCII " " = 32
    if (needle == NULL) // no spaces - bizarre error
    {
        error(400);
        return false;
    }    
    length = needle - haystack;
    char method[length];
    method[length] = '\0';
    strncpy(method, haystack, length);
    ...
    // now determine request target.  Will find HTTP method before determining query.
    haystack = needle + 1;
    needle = strchr(haystack, 32); // find the second space: ASCII " " = 32
    if (needle == NULL) // no more spaces - therefore bad request
    {
        error(414);
        return false;
    }    
    length = needle - haystack;
    char requesttarget[length];
    requesttarget[length] = '\0';
    strncpy(requesttarget, haystack, length);

     // now find the HTTP version
    haystack = needle + 1;
    char version[9];
    strncpy(version, haystack, 8);
    version[8] = '\0';
   ...
   // check request-target has no '"' .. quote marks
    ...
    // now check if there is a query
    haystack = requesttarget;
    needle = strchr(haystack, 63); // find question mark: ASCII ? = 63
    if (needle == NULL)
    {
        length = sizeof(requesttarget);
        strncpy(abs_path, haystack, length);
        abs_path[length] = '\0';
        *query = '\0';
    }
    else
    {
        ...
        abs_path[length] = '\0';
        haystack = needle + 1;
        length = sizeof(requesttarget) - length - 1;
        strncpy(query, haystack, length);
        query[length] = '\0';
    }
    return true;
}
...
void transfer(const char* path, const char* type)
{
    ...
    if (file == NULL)
    {
        printf(" couldn't open file as read only");
        error(500);
        return;
    }
